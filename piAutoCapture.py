...
def capture_and_analyze(camera):
    """
    Capture image with PiCamera and run classification inference
    """
...
def motion_detection_loop():
    """
    Main motion detection loop using PiCamera and OpenCV
    """
    ...
    # Initialize OpenCV for motion detection
    cap = cv2.VideoCapture(0)  # Use PiCamera stream
    ...
    try:
        while True:
            ...
            # Read frame from camera stream only when not in classification
            ret, frame = cap.read()
            if not ret:
            ...
            if motion_detected and not capture_triggered and not classification_in_progress and motion_detected_time == 0:
                ...
                if current_time - motion_detected_time >= capture_delay:
                    ...
                    # Capture and analyze using PiCamera
...
def main():
    print("🚀 Starting SmartSort PiCamera Motion-Based Auto-Capture")
    print("=" * 60)
    print("📋 Features:")
